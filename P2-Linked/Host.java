...
public abstract class Host {
    ...
    private FutureEventList fel; //reference to LinkedEventList object 
    ...
    protected Host() {
        this.activeTimers = new HashMap<>(); //creates a map for timers
        this.neighbor = new HashMap<>(); //creates a map for neighbours of Host
        this.neighborDistance = new HashMap<>(); //creates a map for distance from neighbours
    }
    ...
    public void addNeighbor(Host neighbor, int distance) {
        ...
        this.neighborDistance.put(neighbor.address, distance); //key == neighbour address
        this.neighbor.put(neighbor.address, neighbor); //key == neighbor 
    }
    ...
    protected void sendToNeighbor(Message msg) {
        Host neighbor = this.neighbor.get(msg.getDestAddress()); //gets neighbour with destAddress
        if (neighbor == null) {
            throw new EventException("sendToNeighbor: destination address of message is unknown: " + msg.getDestAddress());
        }

        int distance = this.neighborDistance.get(msg.getDestAddress()); //gets distance
